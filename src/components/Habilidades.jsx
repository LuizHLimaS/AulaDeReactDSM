  ...
  return (
    <section id="habilidades" className="habilidades">
      <div className="container">
        ...
        <ul className="habilidades__barras" aria-label="Barras de Proficîencia">
          {skills.map((s) => (
            <li key={s.nome} className="habilidade">
              <div className="hablidade__topo">
                <span className="habilidade__nome">{s.nome}</span>
                <span className="habilidade__percentual" aria-hidden="true">
                  {s.nivel}%
                </span>
              </div>
              <div
                className="habilidade__bar"
                role="progressbar"
                aria-valuemin={0}
                aria-valuemax={100}
                aria-valuenow={s.nivel}
                aria-label={`Proficiencia em ${s.nome}:${s.nivel}%`}
              >
                <div
                  className="habilidade__fill"
                  style={{ width: `${s.nivel}%` }}
                />
              </div>
            </li>
        ...
        <div className="habilidade__tecnologias">
          {tecnologias.map((t) => (
